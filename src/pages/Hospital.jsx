...
function HospitalWorkspace({ hospital }) {
  ...
  if (requests.data && matches.data) {
    if (readyToConfirm.length) next = <NextStep icon={Handshake} title={readyToConfirm.length === 1 ? `${readyToConfirm[0].donor_label} accepted a match` : `${readyToConfirm.length} donors accepted matches`} body="Confirm each match once medical tests are complete." action={t('Review')} onClick={() => openTab('matches')} />;
    else if (pending.length) next = <NextStep icon={ClipboardText} title={`${pending.length} ${pending.length === 1 ? 'request needs' : 'requests need'} verification`} body="Verify each request and set its priority so the patient enters priority matching." action={t('Review')} onClick={() => select(pending[0].id)} />;
  ...
  return <div className="page-container workspace">
    <PageHeading eyebrow={`${t('Hospital portal')} · ${hospital.city}, ${hospital.state}`} title={hospital.name} />
    {next || <Loading />}
    ...
    <div className="tabs" role="group" aria-label="Hospital view">{[['queue', 'Patient queue'], ['matches', 'Matches'], ['registry', 'Donor registry']].map(([key, label]) => <button key={key} type="button" className={tab === key ? 'active' : ''} aria-pressed={tab === key} onClick={() => openTab(key)}>{t(label)}</button>)}</div>
...
function RequestQueue({ rows, selectedId, onSelect }) {
  ...
  return <section className="queue" aria-label={t('Patient queue')}>
...
function RequestPane({ id, matches, onChanged }) {
  ...
  const facts = [['Requester email', r.email], ['Patient blood group', r.blood_group], ['Patient age', ageFrom(r.patient_dob)], ['Quantity', r.quantity], ['Reported urgency', r.urgency === 'Not Emergency' ? t('Standard') : t(r.urgency)], ['Phone', r.phone], ['Address', `${r.address} ${r.zip}`], ['Priority', r.priority ? capitalize(r.priority) : '—'], ['Clinical score', r.verification === 'verified' ? `${r.clinical_score} of 40` : '—'], ['On verified list since', r.verified_at ? formatDate(r.verified_at) : '—']];
  return <>
    ...
    <section><div className="section-heading compact"><div><span className="eyebrow">Priority matching</span><h2>{t('Ranked donors')}</h2></div></div><Candidates request={r} resource={candidates} onProposed={refresh} /><p className="quiet-note">Rankings are suggestions based on the information supplied. Crossmatching, tissue typing, and eligibility are decided by the transplant team.</p></section>
  </>;
}
  ...
  return <section className="panel"><h2>{request.verification === 'verified' ? 'Update verification' : 'Verify this request'}</h2><p className="muted">Only verified requests enter priority matching. The priority and clinical score you set drive the ranking.</p>
    <form onSubmit={(e) => { e.preventDefault(); const body = formValues(e.currentTarget); action.run(async () => { await api(`/hospital/requests/${request.id}/verification`, { method: 'PATCH', body }); onSaved(); }, { success: body.decision === 'verified' ? 'Request verified. The patient is now in priority matching.' : 'Request rejected. The requester has been notified.' }); }}>
      <div className="fields">
        <Field label="Decision" name="decision" options={[{ value: 'verified', label: t('Verify') }, { value: 'rejected', label: t('Reject') }]} value={decision} onChange={(e) => setDecision(e.target.value)} />
        {decision === 'verified' && <><Field label="Medical priority" name="priority" options={PRIORITIES.map((p) => ({ value: p, label: capitalize(t(p)) }))} defaultValue={request.priority || ''} /><Field label="Clinical score (0–40)" name="clinical_score" type="number" min={0} max={40} defaultValue={request.clinical_score ?? 0} hint="Organ-specific severity, such as MELD (6–40) for liver. Use 0 when none applies." /></>}
      ...
      <div className="form-actions"><button className="button" disabled={action.busy}>{action.busy ? 'Saving…' : t('Save')}<ArrowRight size={18} /></button></div>
    </form></section>;
}
...
function CompareDonors({ candidates }) {
  ...
  return <section aria-label={t('Compare donors')}><h3 className="form-section-title">{t('Compare donors')}</h3><div className="compare">{candidates.slice(0, 3).map((c, i) => <article key={c.pledge_id} className={`compare-card ${i === 0 ? 'best' : ''}`}>
...
function Candidate({ candidate: c, position, request, onProposed }) {
  ...
  return <article className="panel candidate"><div>
    ...
    <form onSubmit={(e) => { e.preventDefault(); const body = { request_id: request.id, pledge_id: c.pledge_id, override_reason: formValues(e.currentTarget).override_reason || '' }; action.run(async () => { await api('/hospital/matches', { method: 'POST', body }); onProposed(); }, { success: c.donor_type === 'living' ? 'Match proposed. The donor has been notified.' : 'Match proposed. After-death consent is already recorded, so you can confirm it in Matches.' }); }}>
      ...
      <Notice>{action.error}</Notice>
      <div className="inline-actions"><button className="button small" disabled={action.busy}>{action.busy ? 'Proposing…' : t('Propose match')}</button></div>
    </form>
...
function MatchesBoard({ resource, onChanged }) {
  ...
  return <div className={`board ${resource.refreshing ? 'refreshing' : ''}`}>{BOARD.map(([key, label, test]) => {
    ...
    return <section key={key} className="board-column" aria-label={t(label)}><h3>{capitalize(t(label))} <span>{items.length}</span></h3>{items.length ? items.map((m) => <MatchCard key={m.id} match={m} resource={resource} onChanged={onChanged} />) : <p className="board-empty">Nothing here</p>}</section>;
  })}</div>;
}
  ...
  return <article className="board-card">
    ...
    {match.status === 'proposed' && <div className="inline-actions">
      {match.donor_response === 'accepted'
        ? <ConfirmButton danger={false} className="button small" title={`Confirm match #${match.id}?`} description="Confirm only after crossmatching and medical tests are complete. The donor and requester will be notified." confirmLabel={t('Confirm after medical tests')} success="Match confirmed." onConfirm={() => decide({ status: 'confirmed', reason: '' })}>{t('Confirm after medical tests')}</ConfirmButton>
...
function DeclineButton({ onDecline }) {
  ...
  const action = useAsync();
  return <>
    <button type="button" className="text-button danger" onClick={() => dialog.current?.showModal()}>{t('Decline')}</button>
    <dialog ref={dialog} className="modal" aria-label="Decline match"><form onSubmit={(e) => { e.preventDefault(); const reason = formValues(e.currentTarget).reason; action.run(async () => { await onDecline(reason); dialog.current?.close(); }, { success: 'Match declined.' }); }}>
      ...
      <div className="modal-actions"><button type="button" className="button secondary" onClick={() => dialog.current?.close()}>{t('Cancel')}</button><button className="button danger-button" disabled={action.busy}>{action.busy ? 'Declining…' : t('Decline')}</button></div>
    </form></dialog>
  </>;
}
...
function DonorRegistry() {
  ...
  return <div className="panel"><h2>{t('Donor registry')}</h2><p>Find an after-death pledge by the donor’s email. Report it available only after death has been certified and consent documented as the law requires. It then enters priority matching from your hospital’s location. Every search is recorded in the audit log.</p>
    <form className="inline-actions" onSubmit={(e) => { e.preventDefault(); lookup(formValues(e.currentTarget).email); }}><Field label="Donor email" name="email" type="email" wide /><button className="button small" disabled={search.busy}><MagnifyingGlass size={16} /> {t('Search')}</button></form>
