...
function RequestCard({ request, matches, showJourney = false }) {
  ...
  return <article className="request-card">
    <div className="request-card-top"><span className="organ-icon"><Heartbeat size={25} /></span><Status value={badge} label={badge === 'pending' ? 'Awaiting hospital' : undefined} /></div>
    ...
    <div className="request-bottom"><span>Quantity <strong>{request.quantity}</strong></span>{owner || user.role === 'admin' ? <Link className="text-link" to={`/requests/${request.id}`}>Review <ArrowUpRight size={17} /></Link> : <Link className="text-link" to={`/pledge?organ=${encodeURIComponent(request.organ)}`}>Pledge to donate <ArrowUpRight size={17} /></Link>}</div>
  </article>;
}
...
function MatchUpdate({ match, resource, onChanged }) {
  ...
  return <article className="panel match-card">
    ...
    {waiting && (declining
      ? <form className="inline-actions" onSubmit={(e) => { e.preventDefault(); respond({ response: 'declined', reason: formValues(e.currentTarget).reason }); }}><Field label="Reason" name="reason" required={false} wide /><button className="button small" disabled={action.busy}>Decline</button><button type="button" className="text-button" onClick={() => setDeclining(false)}>Cancel</button></form>
      : <div className="inline-actions"><button type="button" className="button small" disabled={action.busy} onClick={() => respond({ response: 'accepted' })}>Accept match</button><button type="button" className="text-button danger" disabled={action.busy} onClick={() => setDeclining(true)}>Decline</button></div>)}
  </article>;
}
...
export function Dashboard() {
  ...
  else if (awaitingMe.length) next = <NextStep icon={BellRinging} title={awaitingMe.length === 1 ? 'A hospital proposed you as a donor' : `${awaitingMe.length} matches are waiting for your answer`} body="Review the match and accept or decline it." action="Review" onClick={scrollToMatches} />;
  else if (rejected) next = <NextStep icon={WarningCircle} title={`Your ${rejected.organ.toLowerCase()} request needs attention`} body={rejected.hospital_note || 'The hospital could not verify it.'} action="Review" to={`/requests/${rejected.id}`} />;
  else if (pendingRequest) next = <NextStep calm icon={Hourglass} title={`Waiting for ${pendingRequest.hospital_name || 'the hospital'} to verify your request`} body="You’ll get a notification as soon as it’s verified." action="Review" to={`/requests/${pendingRequest.id}`} />;
  else if (!pledges.data.length && !myRequests.length) next = <NextStep calm icon={Sparkle} title="Pledge to donate, or request an organ for a patient" body="It takes a few minutes, and you can save a draft as you go." action="Pledge to donate" to="/pledge" />;
  ...
  return <div className="page-container workspace">
    <PageHeading eyebrow="Your corner of the community" title={`Welcome, ${user.first_name}.`} action={<ButtonLink to="/pledge">Pledge to donate</ButtonLink>}>A small step today. A little more hope for tomorrow.</PageHeading>
    {next || <Loading />}
    ...
    <div className="section-heading compact" id="match-updates"><div><span className="eyebrow">What’s happening</span><h2>Match updates</h2></div></div>
    ...
    <div className="section-heading compact"><div><span className="eyebrow">Your requests</span><h2>My requests</h2></div><Link className="text-link" to="/requests/new">Request an organ <Plus size={17} /></Link></div>
    ...
    <div className="section-heading compact"><div><span className="eyebrow">Your willingness to help</span><h2>My pledges</h2></div><Link className="text-link" to="/pledge">Pledge to donate <Plus size={17} /></Link></div>
    <Notice>{pledges.error}</Notice>
    {!pledges.data ? <Loading /> : <DataTable columns={pledgeColumns(pledges, refresh)} rows={pledges.data} emptyTitle="You haven’t pledged yet" emptyText="Pledge once, and hospitals can find you as an anonymous donor for patients by medical priority." />}
    <div className="section-heading compact"><div><span className="eyebrow">The community</span><h2>Recent organ requests</h2></div><Link className="text-link" to="/requests">View all <ArrowRight size={17} /></Link></div>
...
export function MyPledges() {
  ...
  return <div className="page-container workspace"><PageHeading title="My pledges" action={<ButtonLink to="/pledge">Pledge to donate</ButtonLink>}>Manage your willingness to donate and follow each pledge.</PageHeading><Notice>{resource.error}</Notice>{!resource.data ? <Loading /> : <DataTable columns={pledgeColumns(resource, resource.refresh)} rows={resource.data} emptyTitle="You haven’t pledged yet" emptyText="Create a pledge to join the donor registry." />}</div>;
}
...
export function MyMatches() {
  ...
  return <div className="page-container workspace"><PageHeading title="Matches">Follow your donation and request journeys.</PageHeading><Notice>{resource.error}</Notice>{!resource.data ? <Loading variant="cards" /> : <>
...
export function RequestDetail() {
  ...
  return <div className="page-container workspace">
    ...
    <PageHeading eyebrow={`Request #${r.id} · ${formatDate(r.created_at)}`} title={`${r.organ} request`} action={owner && r.status === 'open' ? <ButtonLink secondary to={`/requests/${r.id}/edit`}>Edit</ButtonLink> : null}>Requested by {r.first_name} {r.last_name}</PageHeading>
    ...
    <div className="portal-grid">
      ...
      <section><h2 className="form-section-title">Match updates</h2>{!matches.data ? <Loading /> : related.length ? <div className="match-list">{related.map((m) => <MatchUpdate key={m.id} match={m} resource={matches} onChanged={request.refresh} />)}</div> : <Empty title="No matches yet">When the hospital proposes a donor, you’ll see it here and get a notification.</Empty>}</section>
    </div>
    {owner && <div className="inline-actions">
      ...
      <ConfirmButton title="Delete this request?" description="Requests with match history can’t be deleted; close them instead." confirmLabel="Delete" busyLabel="Deleting…" success="Request deleted." onConfirm={async () => { await api(`/requests/${r.id}`, { method: 'DELETE' }); navigate('/requests'); }}>Delete</ConfirmButton>
    </div>}
  </div>;
}
...
function RequestEditor({ initial }) {
  ...
  return <FormShell back={initial ? `/requests/${initial.id}` : '/requests'} backLabel={initial ? 'Back to request' : 'Back to requests'} eyebrow="Share your need with the community" title={initial ? 'Edit your request' : 'Request an organ'} description="The treating hospital verifies your request and sets its medical priority for matching." aside={<p>Your name, hospital, city, and note are visible to signed-in members. The patient’s date of birth and your contact details are shared only with the hospital and administrators.</p>}>
    <Wizard key={formKey} steps={steps} busy={action.busy} error={action.error || hospitals.error} submitLabel={initial ? 'Save changes' : 'Create a request'} draft={initial ? null : draft} onDiscard={() => { draft.clear(); setFormKey((k) => k + 1); }}
...
export function Records() {
  ...
  const columns = [
    ...
    { key: 'action', header: 'Action', sortable: false, render: (r) => <ConfirmButton title="Delete this record?" confirmLabel="Delete" busyLabel="Deleting…" success="Record deleted." onConfirm={async () => { await resource.mutate((rows) => rows.filter((row) => row.id !== r.id), () => api(`/records/${r.id}`, { method: 'DELETE' })); }}>Delete</ConfirmButton> },
  ];
  return <div className="page-container workspace">
